...
//        	store.connect("imap.gmail.com", username, password);
            ...
            message.setSubject("歡迎光臨，KBSHOP");
            ...
            map.put("title", "KBSHOP-歡迎光臨");
            map.put("brand","註冊成功");
            map.put("contextPath", "KBSHOP");            
    ...
    public static void sendOrderMail(String to,int orderId,double amount,String createdTime) {
        if (to == null || !to.matches("^[\\w-\\.]+@([\\w-]+\\.)+[\\w-]{2,4}$")) {
            throw new IllegalArgumentException("email格式不正確!");            
        }
...
//        	store.connect("imap.gmail.com", username, password);
            ...
            // Set Subject: header field
            message.setSubject("KBSHOP，訂單編號:"+orderId+"下訂成功");
            ...
            Map<String, Object> map = new HashMap<>();
            map.put("title", "KBSHOP-下訂成功");
            map.put("brand", "訂單編號為:"+orderId);
            map.put("money", "訂單金額為:"+amount);
            map.put("time", createdTime);
            map.put("contextPath", "KBSHOP");            
            String ipAddress = java.net.InetAddress.getLocalHost().getHostAddress();
            map.put("host", ipAddress);

            String htmlText = MailService.processTemplateWithMap("order.ftl", map);
            messageBodyPart.setContent(htmlText, "text/html;charset=utf-8");
            // add it
            multipart.addBodyPart(messageBodyPart);
    ...
    public static void sendATMPaymentMail(String to,String orderId,String amount,String createdTime) {
        if (to == null || !to.matches("^[\\w-\\.]+@([\\w-]+\\.)+[\\w-]{2,4}$")) {
            throw new IllegalArgumentException("email格式不正確!");            
        }
...
//        	store.connect("imap.gmail.com", username, password);
            ...
            // Set Subject: header field
            message.setSubject("KBSHOP，訂單編號:"+orderId+"付款成功");
            ...
            String htmlText = MailService.processTemplateWithMap("ATM.ftl", map);
            messageBodyPart.setContent(htmlText, "text/html;charset=utf-8");
            // add it
            multipart.addBodyPart(messageBodyPart);
    ...
    public static void sendCreditCardPaymentMail(String to,String orderId,String amount,String createdTime) {
        if (to == null || !to.matches("^[\\w-\\.]+@([\\w-]+\\.)+[\\w-]{2,4}$")) {
            throw new IllegalArgumentException("email格式不正確!");            
        }
...
//        	store.connect("imap.gmail.com", username, password);
            ...
            String htmlText = MailService.processTemplateWithMap("CreditCard.ftl", map);
            messageBodyPart.setContent(htmlText, "text/html;charset=utf-8");
            // add it
            multipart.addBodyPart(messageBodyPart);
